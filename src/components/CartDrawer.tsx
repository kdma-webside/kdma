...
const CartDrawer = () => {
    ...
    const handleCheckout = async () => {
        ...
        try {
            // Create order in backend primarily
            const { createOrder, updateOrderStatus } = await import('@/app/actions/orders');
            ...
            if (!order) {
                alert('Failed to initiate deployment (Order Creation Failed).');
            ...
            processPayment({
                ...
                currency: 'INR',
                name: 'KDMA Academy',
                description: `Academy Recruitment: ${cartItems.length} items`,
                image: 'https://raw.githubusercontent.com/lucide-react/lucide/main/icons/shield.svg',
                order_id: order.id, // Pass our internal order id as reference if needed, though Razorpay has its own order_id system. 
                // Better to put it in notes for complete clarity if we aren't using Razorpay Orders API fully yet.
                notes: {
                ...
                handler: async (response: any) => {
                    ...
                    alert(`Legion Deployment Successful! ID: ${response.razorpay_payment_id}`);
                    clearCart();
                    setIsCartOpen(false);
                    setIsCheckingOut(false);
            ...
            alert('An error occurred during deployment initialization.');
            setIsCheckingOut(false);
        }
    };
                        ...
                        {cartItems.length > 0 && (
                            <div className="p-8 bg-[#0C0C0C] border-t border-white/10 space-y-6">
                                ...
                                <p className="text-center text-[8px] text-white/20 font-black tracking-[0.3em] uppercase">
                                    RAZORPAY SECURE ENCRYPTION ACTIVE
                                </p>
                            </div>
                        )}
                    </motion.div>
